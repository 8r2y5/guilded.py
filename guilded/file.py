...
class File:
    def __init__(
        self, fp: Union[str, io.BufferedIOBase], *, file_type: FileType = None
    ):
        ...
        if type(fp) == str:
            ...
            if file_type is None:
                try:
                    ...
                    raise ValueError(
                        "File must have an extension if file_type is not specified."
                    )
                else:
                    if extension in utils.valid_image_extensions:
                        ...
                        raise TypeError(
                            "Invalid file type. Consider passing file_type to manually tell Guilded what type of file this is."
                        )
