...
class Group:
    def __init__(self, *, state, team, data):
        ...
        self.created_by = self.team.get_member(
            data.get("createdBy")
        ) or data.get("createdBy")
        self.updated_by = self.team.get_member(
            data.get("updatedBy")
        ) or data.get("updatedBy")
        self.archived_by = self.team.get_member(
            data.get("archivedBy")
        ) or data.get("archivedBy")
    ...
    async def edit(self, **fields):
        if isinstance(fields.get("icon"), str):
            fields["icon_url"] = fields.get("icon")
        elif isinstance(fields.get("icon"), File):
            file = fields.get("icon")
            file.type = MediaType.group_icon
            fields["icon_url"] = await file._upload()
        elif not fields.get("icon"):
            fields["icon_url"] = None

        return await self._state.update_team_group(
            self.team.id, self.id, **fields
        )
