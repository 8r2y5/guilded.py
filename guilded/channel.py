...
class Thread(guilded.abc.TeamChannel):
    def __init__(self, **fields):
        ...
        if data.get("type").lower() == "team":
            self.team = fields.get("team") or self._state._get_team(
                data.get("teamId")
            )
            team_id = getattr(self.team, "id", None) or data.get("teamId")
            self.parent = fields.get(
                "parent"
            ) or self._state._get_team_channel(data.get("parentChannelId"))
            self.created_by = fields.get(
                "created_by"
            ) or self._state._get_team_member(team_id, data.get("createdBy"))
            for user in data.get("participants"):
                user = self._state._get_team_member(
                    team_id, data.get("createdBy")
                )
                if user is not None:
            ...
            self.team = None
            self.parent = fields.get("parent") or self._state._get_dm_channel(
                data.get("parentChannelId")
            )
            self.created_by = fields.get(
                "created_by"
            ) or self._state._get_user(data.get("createdBy"))
            for user in data.get("participants"):
