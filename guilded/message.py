...
class Message:
                    ...
                    if element["object"] == "inline":
                        if element["type"] == "mention":
                            ...
                            if mentioned["type"] == "role":
                                ...
                                if (
                                    mentioned.get("nickname") is True
                                    and mentioned.get("matcher") is not None
                                ):
                                    name = (
                                        name.strip("@").strip(name).strip("@")
                                    )
                                    if not name.strip():
    ...
    async def delete(self):
        return await self._state.delete_message(
            self.channel.id or self.channel_id, self.id
        )
    ...
    async def send(
        ...
        *,
        ...
        payload = {
            ...
            "old_files": [
                await attachment.to_file() for attachment in self.attachments
            ],
        }
        if content:
